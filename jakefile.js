...
var fs = require('fs'), path = require('path'), uglify = require('uglify-js'), spawn = require('child_process'),
  copy, concat, minify, replace, del, zip;
...
desc("This is the core task");
task("core",["init"],function(){
...
desc("initializes all directories");
task("init",function(){
  directory(build);
  directory(build+"/calendars");
  directory(build+"/zoneinfo");
  directory(buildnode);
  directory(buildnode+"/calendars");
  directory(buildnode+"/zoneinfo");
  directory(caldest);
  directory(dist);
});
...
task("zoneinfo",["init"],function(){
  var co = spawn.spawn(zonecompiler,[zoneinfo+"/"+zoneversion,zonebuild],{env:{RUBYLIB:src+"/zoneinfo"}});
  co.on('exit',function(){
